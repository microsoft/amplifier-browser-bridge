...
2. Packs that staged directory into a real CRX3 via a real Chromium/Chrome/Edge
...
4. Writes the result to `dist/android/amplifier-browser-bridge-android-v<version>.crx`
   (`dist/` is gitignored -- this is a build artifact, not source).
5. Runs `scripts/verify_crx.py` automatically and prints the result.
